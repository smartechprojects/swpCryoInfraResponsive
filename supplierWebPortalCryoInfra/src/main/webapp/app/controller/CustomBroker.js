Ext.define('SupplierApp.controller.CustomBroker', {
    extend: 'Ext.app.Controller',
    stores: ['CustomBroker'],
    models: ['CustomBroker'],
    views: ['customBroker.CustomBrokerPanel','customBroker.CustomBrokerGrid','customBroker.CustomBrokerForm',
			'customBroker.CustomBrokerDetailPanel', 'customBroker.ForeignCustomBrokerForm','customBroker.ComplementoPagoPanelCB',
			'customBroker.AcceptInvGridCB','customBroker.SelInvGridCB'],
    refs: [
	    {
	        ref: 'customBrokerGrid',
	        selector: 'customBrokerGrid'
	    },
	    {
	        ref: 'customBrokerForm',
	        selector: 'customBrokerForm'
	    },
	    {
	    	ref: 'foreignCustomBrokerForm',
	    	selector: 'foreignCustomBrokerForm'
	    },
	    {
	    	ref: 'complementoPagoPanelCB',
	    	selector: 'complementoPagoPanelCB'
	    },
	    {
	    	ref: 'acceptInvGridCB',
	    	selector: 'acceptInvGridCB'
	    },
	    {
	    	ref: 'selInvGridCB',
	    	selector: 'selInvGridCB'
	    }],
    ...
    init: function() {
        ...
        this.control({
				...
				'customBrokerGrid button[action=uploadNewFiscalDocCB]' : {
					click : this.invLoad
				},
				'foreignCustomBrokerForm button[action=uploadForeignDocsCB]' : {
					click: this.uploadForeignAdditional
				},
				'foreignCustomBrokerForm button[action=sendForeignRecordCB]' : {
				...
				'#approveInvoiceCB' : {
					"buttonclick" : this.approveInvoiceCB
				},
				'#rejectInvoiceCB' : {
					"buttonclick" : this.rejectInvoiceCB
				},
				'#acceptSelInvCB' : {
					"buttonclick" : this.acceptSelInvCB
				},
				'#rejectSelInvCB' : {
					"buttonclick" : this.rejectSelInvCB
				},
				'acceptInvGridCB button[action=loadComplFileCB]' : {
					click: this.loadComplFileCB
				},
				'customBrokerGrid button[action=cbSearch]' : {
					click : this.cbSearch
				},
				'customBrokerGrid button[action=poUploadInvoiceFileCB]' : {
					click : this.poUploadInvoiceFileCB
				},
				'customBrokerGrid button[action=poUploadCreditNoteFileCB]' : {
					click : this.poUploadCreditNoteFileCB
				},
				'customBrokerGrid button[action=cbLoadCompl]' : {
					click : this.cbLoadCompl
				},
        });
    },
    ...
    gridSelectionChange: function(model, record) {
        if (record) {
        	...
        	this.winDetail.show();        	
        	var form = this.getCustomBrokerForm().getForm();
        	form.loadRecord(record);

        	//Ext.getCmp('fileListHtmlInvoice').setValue("");
        	
        	Ext.Ajax.request({
				url : 'documents/listConceptDocumentsByFiscalRefCB.action',
				method : 'GET',
					params : {
						start : 0,
						limit : 20,
    ...
    approveInvoiceCB : function(grid, rowIndex, colIndex, record) {
    	var me = this;
    	var record = grid.store.getAt(rowIndex);
    	var orderNumber = record.data.orderNumber;
    	var dto = Ext.create('SupplierApp.model.CustomBroker',record.data);
    	var status ="APROBADO";
				...
				'Ext.form.Panel',
				{
					...
					buttons : [{
						...
						handler : function() {
							...
							if (form.isValid()) {
								var centroCostos = Ext.getCmp('centroCostosCB').getValue();
								var concepto = Ext.getCmp('conceptoArticuloCB').getValue();
								var company = Ext.getCmp('companyCB').getValue();
								var note = Ext.getCmp('notesCB').getValue();
									...
									fn : function(btn, text) {
										if (btn === 'yes') {
											if(text != ""){
												...
												//var notes = text;
												Ext.Ajax.request({
												    url: 'customBroker/update.action',
												    method: 'POST',
												    params: {
    ...
    rejectInvoiceCB : function(grid, rowIndex, colIndex, record) {
    	var me = this;
    	var record = grid.store.getAt(rowIndex);
    	var dto = Ext.create('SupplierApp.model.CustomBroker',record.data);
    ...
    poUploadInvoiceFileCB:  function(button) {
    	var supNumber = Ext.getCmp('supNumberCB').getValue() == ''?'':Ext.getCmp('supNumberCB').getValue();
    ...
    poUploadCreditNoteFileCB:  function(button) {
    	var supNumber = Ext.getCmp('supNumberCB').getValue() == ''?'':Ext.getCmp('supNumberCB').getValue();
    	    		    ...
    	    		    success: function(fp, o) {
    	    	    		...
    	    	    		if(sup.data.country.trim() == "MX"){// || me.isStorageOnly == true
    	    			    					...
    	    			    					'Ext.form.Panel',
    	    			    					{
    	    			    						...
    	    			    						items : [{
    		    			    						        ...
    		    			    						        items:[{
    		    			    									...
    		    			    									items : [{
	        		    			    									...
	        		    			    									id : 'supCompanyComboCB',
	        		    			    									itemId : 'supCompanyComboCB',
	        		    			    									name : 'supCompany',
    	    			    						...
    	    			    						buttons : [{
    	    			    							...
    	    			    							handler : function() {
    	    			    								...
    	    			    								if (form.isValid()) {
    	    			    									form.submit({
    	    			    												url : 'customBroker/uploadInvoiceWithoutOrder.action',
    	    			    												waitMsg : SuppAppMsg.supplierLoadFile,
    	    			    												success : function(fp, o) {
    	    			    													var res = Ext.decode(o.response.responseText);
    				    		...
    				    		me.winLoadInv = new Ext.Window({
    					    		...
    					    		items : [ 
    					    				{
    					    				xtype:'foreignCustomBrokerForm'
    					    				} 
    					    			]
    					    	});
    				    		
    				    		//orderForm.findField('addressNumber').getValue(),
    				        	var foreignForm = me.getForeignCustomBrokerForm().getForm();
    ...
    uploadForeignAdditional : function(button) {
    	var me = this;
        var form = this.getForeignCustomBrokerForm().getForm();
    	if (form.isValid()) {
					...
					'Ext.form.Panel',
					{
						...
						buttons : [ {
							...
							handler : function() {
								...
								if (form.isValid()) {
									form.submit({
												url : 'uploadForeignAdditionalCB.action',
												waitMsg : SuppAppMsg.supplierLoadFile,
												success : function(fp, o) {
													var res = Ext.decode(o.response.responseText);
													...
													Ext.Ajax.request({														
														...
														method : 'GET',
															...
															success : function(response,opts) {
																...
																Ext.getCmp('fileListForeignHtmlCB').setValue(files);
																Ext.getCmp('attachmentFlagCB').setValue("ATTACH");																																
															},
															failure : function() {
															}
														});
    ...
    sendForeignRecord : function(button) {
    	var me = this;
    	var form = this.getForeignCustomBrokerForm().getForm();
    	var attach = Ext.getCmp('attachmentFlagCB').getValue();
    	...
    	if (form.isValid()) {
			...
			form.submit({
				url : 'supplier/orders/uploadForeignInvoiceWithoutOrderCB.action',
				waitMsg : SuppAppMsg.supplierProcessRequest,
				success : function(fp, o) {
					var res = Ext.decode(o.response.responseText);
					if(me.getForeignCustomBrokerForm().getForm()){
						me.getForeignCustomBrokerForm().getForm().reset();
					}
					me.winLoadInv.destroy();
					Ext.MessageBox.alert({ maxWidth: 700, minWidth: 650, title: SuppAppMsg.supplierMsgValidationLoad, msg: SuppAppMsg.approvalInvRespUploaded});					
				},
    ...
    acceptSelInvCB : function(grid, record) {
    	var gridAccept = this.getAcceptInvGridCB();
    ...
    rejectSelInvCB : function(grid, record) {
    	var gridAccept = this.getAcceptInvGridCB();
    	var storeAccept = gridAccept.getStore();
		storeAccept.remove(record);
    },
    
    loadComplFileCB : function(grid, record) {
        var me = this;    	
    	var customBrokerArray = [];
    	var gridAccept = this.getAcceptInvGridCB();
    	var storeAccept = gridAccept.getStore();
    	
    	var gridSelect = this.getSelInvGridCB();
    	var storeSelect = gridSelect.getStore();
    	
    	var supNumber = Ext.getCmp('supNumberCB').getValue() == ''?'':Ext.getCmp('supNumberCB').getValue();
		storeAccept.each(function(rec) {
			if(rec){
				customBrokerArray.push(rec.data.id);
			}
		});
		
		if(customBrokerArray.length > 0){
			var filePanel = Ext.create(
					'Ext.form.Panel',
					{
						width : 900,
    ...
    cbSearch: function(button) {
    	var grid = this.getCustomBrokerGrid();
    	var store = grid.getStore();
    	//var poNumber = Ext.getCmp('poNumber').getValue() ==''?'0':Ext.getCmp('poNumber').getValue();
    	var supNumber = Ext.getCmp('supNumberCB').getValue() == ''?'':Ext.getCmp('supNumberCB').getValue();
    	var cbUUID = Ext.getCmp('cbUUID').getValue() == ''?'':Ext.getCmp('cbUUID').getValue();
    	var documentType = Ext.getCmp('comboDocumentTypeCB').getValue();
    	var invoiceStatus = Ext.getCmp('comboInvoiceStatusCB').getValue();
    ...
    cbLoadCompl: function(button) {    	
    	...
    	if(supNumber != ""){
    		...
    		var gridSel = this.getSelInvGridCB();
        	var gridAccept = this.getAcceptInvGridCB();
