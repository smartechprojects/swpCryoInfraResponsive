package com.eurest.supplier.edi;

public class ReceivingAdviceDetailDTO {
	
	private String SZEDTY; // Default 2
	private int SZEDSQ; // Default 1
	
	private int SZEDOC;
	private String SZEDCT;
	private String SZEKCO;
	private double SZEDLN;
	private String SZEDST;
	private String SZEDER;
	
	private int SZDOCO;
	private String SZDCTO;
	private String SZKCOO;
	private String SZSFXO;
	
	private double SZLNID;
	private String SZLITM;
	private String SZLSTS; // Ver UDC (47 LS), Valores: 1,7 o 9
	private double SZUREC;
	private double SZAREC;
	private double SZPRRC;
	private double SZUORG;
	private String SZUOM;
	
	private int SZAN8;
	private int SZSHAN;
	private String SZMCU;
	
	private String TORG;
	private String USER;
	private String PID;
	private String JOBN;
	private String UPMJ;
	private int TDAY;
	
	public int getSZEDOC() {
		return SZEDOC;
	}
	public void setSZEDOC(int sZEDOC) {
		SZEDOC = sZEDOC;
	}
	public String getSZEDCT() {
		return SZEDCT;
	}
	public void setSZEDCT(String sZEDCT) {
		SZEDCT = sZEDCT;
	}
	public String getSZEKCO() {
		return SZEKCO;
	}
	public void setSZEKCO(String sZEKCO) {
		SZEKCO = sZEKCO;
	}
	public double getSZEDLN() {
		return SZEDLN;
	}
	public void setSZEDLN(double sZEDLN) {
		SZEDLN = sZEDLN;
	}
	public String getSZEDST() {
		return SZEDST;
	}
	public void setSZEDST(String sZEDST) {
		SZEDST = sZEDST;
	}
	public String getSZEDER() {
		return SZEDER;
	}
	public void setSZEDER(String sZEDER) {
		SZEDER = sZEDER;
	}
	public int getSZDOCO() {
		return SZDOCO;
	}
	public void setSZDOCO(int sZDOCO) {
		SZDOCO = sZDOCO;
	}
	public String getSZDCTO() {
		return SZDCTO;
	}
	public void setSZDCTO(String sZDCTO) {
		SZDCTO = sZDCTO;
	}
	public String getSZKCOO() {
		return SZKCOO;
	}
	public void setSZKCOO(String sZKCOO) {
		SZKCOO = sZKCOO;
	}
	public String getSZSFXO() {
		return SZSFXO;
	}
	public void setSZSFXO(String sZSFXO) {
		SZSFXO = sZSFXO;
	}
	public double getSZLNID() {
		return SZLNID;
	}
	public void setSZLNID(double sZLNID) {
		SZLNID = sZLNID;
	}
	public String getSZLSTS() {
		return SZLSTS;
	}
	public void setSZLSTS(String sZLSTS) {
		SZLSTS = sZLSTS;
	}
	public double getSZUREC() {
		return SZUREC;
	}
	public void setSZUREC(double sZUREC) {
		SZUREC = sZUREC;
	}
	public double getSZAREC() {
		return SZAREC;
	}
	public void setSZAREC(double sZAREC) {
		SZAREC = sZAREC;
	}
	public double getSZPRRC() {
		return SZPRRC;
	}
	public void setSZPRRC(double sZPRRC) {
		SZPRRC = sZPRRC;
	}
	public String getSZEDTY() {
		return SZEDTY;
	}
	public void setSZEDTY(String sZEDTY) {
		SZEDTY = sZEDTY;
	}
	public int getSZEDSQ() {
		return SZEDSQ;
	}
	public void setSZEDSQ(int sZEDSQ) {
		SZEDSQ = sZEDSQ;
	}
	public int getSZAN8() {
		return SZAN8;
	}
	public void setSZAN8(int sZAN8) {
		SZAN8 = sZAN8;
	}
	public int getSZSHAN() {
		return SZSHAN;
	}
	public void setSZSHAN(int sZSHAN) {
		SZSHAN = sZSHAN;
	}
	public String getSZMCU() {
		return SZMCU;
	}
	public void setSZMCU(String sZMCU) {
		SZMCU = sZMCU;
	}
	public String getTORG() {
		return TORG;
	}
	public void setTORG(String tORG) {
		TORG = tORG;
	}
	public String getUSER() {
		return USER;
	}
	public void setUSER(String uSER) {
		USER = uSER;
	}
	public String getPID() {
		return PID;
	}
	public void setPID(String pID) {
		PID = pID;
	}
	public String getJOBN() {
		return JOBN;
	}
	public void setJOBN(String jOBN) {
		JOBN = jOBN;
	}
	public String getUPMJ() {
		return UPMJ;
	}
	public void setUPMJ(String uPMJ) {
		UPMJ = uPMJ;
	}
	public int getTDAY() {
		return TDAY;
	}
	public void setTDAY(int tDAY) {
		TDAY = tDAY;
	}
	public String getSZLITM() {
		return SZLITM;
	}
	public void setSZLITM(String sZLITM) {
		SZLITM = sZLITM;
	}
	public double getSZUORG() {
		return SZUORG;
	}
	public void setSZUORG(double sZUORG) {
		SZUORG = sZUORG;
	}
	public String getSZUOM() {
		return SZUOM;
	}
	public void setSZUOM(String sZUOM) {
		SZUOM = sZUOM;
	}
	
	
	
	
}
