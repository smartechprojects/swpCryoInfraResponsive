package com.eurest.supplier.edi;

public class VoucherDetailDTO {
	
	private String SZEDTY; // Default 2
	private int SZEDSQ; // Default 1
	private String SZEKCO;
	private int SZEDOC;
	private String SZEDCT;
	private double SZEDLN;
	private String SZEDST; //Default to 810
	private String SZEDER; // Default to R
	private int SZEDDL;
	private String SZKCOO;
	private int SZDOCO;
	private String SZDCTO;
	private String SZSFXO;
	private double SZLNID;
	private String SZLNTY;
	private int SZAN8;
	private String SZVINV;
	private String SZLITM;
	private String SZMCU;
	private String SZAVCH; //Default to N
	private String SZAID;
	private String SZANI;
	private double SZUOPN;
	private String SZUOM;
	private double SZUPRC;
	private double SZAEXP;
	private double SZAG;
	private double SZAAP;
	private String TORG;
	private String USER;
	private String PID;
	private String JOBN;
	private String UPMJ;
	private int TDAY;
	private String SZCRCD;
	private String CRRM;
	private int URAB;
	private double SZCRR;
	private double SZFRRC;
	private double SZACR;
	private double SZFAP;
	private int SZDDJ;
	private String SZPST;
	private String SZDCTV;
	private String SZVR01;
	private String SZVR02;
	private String SZURRF;
	private String SZEDBT;
	
	private String SZEDSP;
    private String SZTORG;
    private String SZUSER;
    private String SZPID;
    private String SZJOBN;
    private String SZUPMJ;
    private String SZRMK;
	
	public String getSZEDTY() {
		return SZEDTY;
	}
	public void setSZEDTY(String sZEDTY) {
		SZEDTY = sZEDTY;
	}
	public int getSZEDSQ() {
		return SZEDSQ;
	}
	public void setSZEDSQ(int sZEDSQ) {
		SZEDSQ = sZEDSQ;
	}
	public String getSZEKCO() {
		return SZEKCO;
	}
	public String getSZANI() {
		return SZANI;
	}
	public void setSZANI(String sZANI) {
		SZANI = sZANI;
	}
	public void setSZEKCO(String sZEKCO) {
		SZEKCO = sZEKCO;
	}
	public int getSZEDOC() {
		return SZEDOC;
	}
	public void setSZEDOC(int sZEDOC) {
		SZEDOC = sZEDOC;
	}
	public String getSZEDCT() {
		return SZEDCT;
	}
	public void setSZEDCT(String sZEDCT) {
		SZEDCT = sZEDCT;
	}
	public double getSZEDLN() {
		return SZEDLN;
	}
	public void setSZEDLN(double sZEDLN) {
		SZEDLN = sZEDLN;
	}
	public String getSZEDST() {
		return SZEDST;
	}
	public void setSZEDST(String sZEDST) {
		SZEDST = sZEDST;
	}
	public String getSZEDER() {
		return SZEDER;
	}
	public void setSZEDER(String sZEDER) {
		SZEDER = sZEDER;
	}
	public int getSZEDDL() {
		return SZEDDL;
	}
	public void setSZEDDL(int sZEDDL) {
		SZEDDL = sZEDDL;
	}
	public String getSZKCOO() {
		return SZKCOO;
	}
	public void setSZKCOO(String sZKCOO) {
		SZKCOO = sZKCOO;
	}
	public int getSZDOCO() {
		return SZDOCO;
	}
	public void setSZDOCO(int sZDOCO) {
		SZDOCO = sZDOCO;
	}
	public String getSZDCTO() {
		return SZDCTO;
	}
	public void setSZDCTO(String sZDCTO) {
		SZDCTO = sZDCTO;
	}
	public String getSZSFXO() {
		return SZSFXO;
	}
	public void setSZSFXO(String sZSFXO) {
		SZSFXO = sZSFXO;
	}
	public double getSZLNID() {
		return SZLNID;
	}
	public void setSZLNID(double sZLNID) {
		SZLNID = sZLNID;
	}
	public String getSZLNTY() {
		return SZLNTY;
	}
	public void setSZLNTY(String sZLNTY) {
		SZLNTY = sZLNTY;
	}
	public int getSZAN8() {
		return SZAN8;
	}
	public void setSZAN8(int sZAN8) {
		SZAN8 = sZAN8;
	}
	public String getSZVINV() {
		return SZVINV;
	}
	public void setSZVINV(String sZVINV) {
		SZVINV = sZVINV;
	}
	public String getSZLITM() {
		return SZLITM;
	}
	public void setSZLITM(String sZLITM) {
		SZLITM = sZLITM;
	}
	public String getSZMCU() {
		return SZMCU;
	}
	public void setSZMCU(String sZMCU) {
		SZMCU = sZMCU;
	}
	public String getSZAVCH() {
		return SZAVCH;
	}
	public void setSZAVCH(String sZAVCH) {
		SZAVCH = sZAVCH;
	}
	public String getSZAID() {
		return SZAID;
	}
	public void setSZAID(String sZAID) {
		SZAID = sZAID;
	}
	public double getSZUOPN() {
		return SZUOPN;
	}
	public void setSZUOPN(double sZUOPN) {
		SZUOPN = sZUOPN;
	}
	public double getSZUPRC() {
		return SZUPRC;
	}
	public void setSZUPRC(double sZUPRC) {
		SZUPRC = sZUPRC;
	}
	public double getSZAEXP() {
		return SZAEXP;
	}
	public void setSZAEXP(double sZAEXP) {
		SZAEXP = sZAEXP;
	}
	public double getSZAG() {
		return SZAG;
	}
	public void setSZAG(double sZAG) {
		SZAG = sZAG;
	}
	public double getSZAAP() {
		return SZAAP;
	}
	public void setSZAAP(double sZAAP) {
		SZAAP = sZAAP;
	}
	public String getTORG() {
		return TORG;
	}
	public void setTORG(String tORG) {
		TORG = tORG;
	}
	public String getUSER() {
		return USER;
	}
	public void setUSER(String uSER) {
		USER = uSER;
	}
	public String getPID() {
		return PID;
	}
	public void setPID(String pID) {
		PID = pID;
	}
	public String getJOBN() {
		return JOBN;
	}
	public void setJOBN(String jOBN) {
		JOBN = jOBN;
	}
	public String getUPMJ() {
		return UPMJ;
	}
	public void setUPMJ(String uPMJ) {
		UPMJ = uPMJ;
	}
	public int getTDAY() {
		return TDAY;
	}
	public void setTDAY(int tDAY) {
		TDAY = tDAY;
	}
	public String getSZUOM() {
		return SZUOM;
	}
	public void setSZUOM(String sZUOM) {
		SZUOM = sZUOM;
	}
	public String getSZCRCD() {
		return SZCRCD;
	}
	public void setSZCRCD(String sZCRCD) {
		SZCRCD = sZCRCD;
	}
	public String getCRRM() {
		return CRRM;
	}
	public void setCRRM(String cRRM) {
		CRRM = cRRM;
	}
	public int getURAB() {
		return URAB;
	}
	public void setURAB(int uRAB) {
		URAB = uRAB;
	}
	public double getSZCRR() {
		return SZCRR;
	}
	public void setSZCRR(double sZCRR) {
		SZCRR = sZCRR;
	}
	public double getSZFRRC() {
		return SZFRRC;
	}
	public void setSZFRRC(double sZFRRC) {
		SZFRRC = sZFRRC;
	}
	public double getSZACR() {
		return SZACR;
	}
	public void setSZACR(double sZACR) {
		SZACR = sZACR;
	}
	public double getSZFAP() {
		return SZFAP;
	}
	public void setSZFAP(double sZFAP) {
		SZFAP = sZFAP;
	}
	public int getSZDDJ() {
		return SZDDJ;
	}
	public void setSZDDJ(int sZDDJ) {
		SZDDJ = sZDDJ;
	}
	public String getSZPST() {
		return SZPST;
	}
	public void setSZPST(String sZPST) {
		SZPST = sZPST;
	}
	public String getSZDCTV() {
		return SZDCTV;
	}
	public void setSZDCTV(String sZDCTV) {
		SZDCTV = sZDCTV;
	}
	public String getSZVR01() {
		return SZVR01;
	}
	public void setSZVR01(String sZVR01) {
		SZVR01 = sZVR01;
	}
	public String getSZURRF() {
		return SZURRF;
	}
	public void setSZURRF(String sZURRF) {
		SZURRF = sZURRF;
	}
	public String getSZEDBT() {
		return SZEDBT;
	}
	public void setSZEDBT(String sZEDBT) {
		SZEDBT = sZEDBT;
	}
	public String getSZEDSP() {
		return SZEDSP;
	}
	public void setSZEDSP(String sZEDSP) {
		SZEDSP = sZEDSP;
	}
	public String getSZTORG() {
		return SZTORG;
	}
	public void setSZTORG(String sZTORG) {
		SZTORG = sZTORG;
	}
	public String getSZUSER() {
		return SZUSER;
	}
	public void setSZUSER(String sZUSER) {
		SZUSER = sZUSER;
	}
	public String getSZPID() {
		return SZPID;
	}
	public void setSZPID(String sZPID) {
		SZPID = sZPID;
	}
	public String getSZJOBN() {
		return SZJOBN;
	}
	public void setSZJOBN(String sZJOBN) {
		SZJOBN = sZJOBN;
	}
	public String getSZUPMJ() {
		return SZUPMJ;
	}
	public void setSZUPMJ(String sZUPMJ) {
		SZUPMJ = sZUPMJ;
	}
	public String getSZRMK() {
		return SZRMK;
	}
	public void setSZRMK(String sZRMK) {
		SZRMK = sZRMK;
	}
	public String getSZVR02() {
		return SZVR02;
	}
	public void setSZVR02(String sZVR02) {
		SZVR02 = sZVR02;
	}	
	
	

}
