...
public class MiddlewareService {
			    	  ...
			    	  DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");   
							...
							for (Receipt o : objList) {										
								...
								if(s!=null) {
									...
									//Multiusuarios
									String emailUsers = userService.getMailFromUsersByAddressNumber(o.getAddressNumber());
									...
									emailContent = emailContent.replace("_DATE_", strDate);
 	 	 							...
 	 	 							emailAsyncSup.setProperties(
 	 	 									...
 	 	 									stringUtils.prepareEmailContent(emailContent + AppConstants.EMAIL_PORTAL_LINK), emailUsers);
			...
			if (poList != null) {
				if (poList.size() > 0) {
					...
					for(Receipt po : poList) {
						...
						List<FiscalDocuments> fdList = fiscalDocumentService.getFiscalDocuments("", "", po.getUuid(), "", 0, 500);
						if (fdList != null) {
							if (fdList.size() > 0) {
								for(FiscalDocuments fd : fdList) {
									if(fd.getUuidFactura().equals(po.getUuid())) {
										fd.setPaymentReference(po.getPaymentReference());
										fd.setPaymentAmount(po.getPaymentAmount());
										fd.setPaymentDate(po.getPaymentDate());
										...
										log4j.info("* fiscalDocumentService.updateDocument uuid: " + po.getUuid() );
									}
								}
							}
						}
		...
		try{
	          ...
	          if(addressNumber==null) {
	            ...
	            if(s.getUpdateApprovalFlow() == null || s.getUpdateApprovalFlow().equals("")) {
	              ...
	              u.setAddressNumber(s.getAddresNumber());
	              u.setUserRole(role);
	...
	public Map<String, Object> getPostVoucher(int start, int limit){
		try{
			...
			if (!fiscalDocList.isEmpty()) {
				...
				for (FiscalDocuments fisDoc : fiscalDocList) {
					if (fisDoc.getUuidFactura() != null && !"".equals(fisDoc.getUuidFactura())) {
		        		...
		        		Users usuarioaprob=userService.getByUserName(fisDoc.getCurrentApprover());
		        		
		        		// obtencion y administracion de batchnumber
		        		
		        		int nextNbr=0;
		        		Date fecha=usuarioaprob.getBacthDate();
		        		if (fecha == null) {
		        		    // Obtener la fecha actual
		        		    Calendar calendar = Calendar.getInstance();
		        		    // Restar un día para obtener la fecha de ayer
		        		    calendar.add(Calendar.DATE, -1);
		        		    // Establecer la fecha a ayer
		        		    fecha = calendar.getTime();
		        		}
		        		
		        		Date now = new Date(); // Fecha y hora actuales
		        		long differenceInMillis = now.getTime() - fecha.getTime();
		        		long differenceInHours = TimeUnit.MILLISECONDS.toHours(differenceInMillis);
		        		if (usuarioaprob.getBacthDate()==null || differenceInHours > 2) {
		        		    System.out.println("La fecha tiene más de dos horas.");
		        		    
		        		    NextNumber nn = nextNumberService.getNextNumber(AppConstants.NN_MODULE_VOUCHER);
		    				nextNbr = nn.getNexInt();
		    				int nextNbrUpd = nextNbr + 1;
		    				nn.setNexInt(nextNbrUpd);
		    				nextNumberService.updateNextNumber(nn);
		    				usuarioaprob.setBatchnumber(nextNbrUpd);
		    				usuarioaprob.setBacthDate(new Date());
		    				usuarioaprob.setLinenumber(1);
		    				
		        		    
		        		    
		        		} else {
		        		    System.out.println("La fecha tiene menos de dos horas.");
		        		    nextNbr=usuarioaprob.getBatchnumber();
		        		    usuarioaprob.setLinenumber(usuarioaprob.getLinenumber()==0?1:usuarioaprob.getLinenumber()+1);
		        		}
		    			
		        		userService.update(usuarioaprob, new Date(), "12345678");
		        		
		        		@SuppressWarnings("unused")
						VoucherHeaderDTO voucher = createNewVoucher(po,inv,0,s,receiptArray,AppConstants.NN_MODULE_VOUCHER,fisDoc);
			...
			////////// notas de credito obtencion para posteo en JDE
			fiscalDocList = fiscalDocumentService.getPendingReplicationInvoice("PENDING","NOTACREDITO",true,start, limit);
			if (!fiscalDocList.isEmpty()) {
				 
				for (FiscalDocuments fisDoc : fiscalDocList) {
					if (fisDoc.getUuidFactura() != null && !"".equals(fisDoc.getUuidFactura())) {
						UserDocument userDocument = documentsService.getInvoiceDocumentByUuid(fisDoc.getUuidFactura(),"NotaCredito");
						if(userDocument != null) {
		        		...
		        		//List<Receipt> receiptArray = purchaseOrderService.getOrderReceipts(fisDoc.getOrderNumber(), userDocument.getAddressBook(), userDocument.getDocumentType(), po.getOrderCompany());
						List<Receipt> receiptArrayNC= purchaseOrderService.getReceiptsByUUID(inv.getCfdiRelacionado());
		        		List<Receipt> receiptArray = purchaseOrderService.getReceiptsByUUID(fisDoc.getUuidFactura());
		        		receiptArray=receiptArray.size()==0?receiptArrayNC:receiptArray;
		        		PurchaseOrder poOriginal=purchaseOrderService.getOrderByOrderAndAddresBook(receiptArray.get(0).getOrderNumber(), userDocument.getAddressBook(), receiptArray.get(0).getOrderType());
//		        		
//		        		if((receiptArrayNC.size()==0&&receiptArray.size()==0)||poOriginal==null) {
//		        			continue;
//		        		}
		        		
		        		List<PurchaseOrderDetail> purchaseordedetail=purchaseOrderService.getCreditNotes(po.getOrderNumber(),  po.getAddressNumber(), "OP",  po.getOrderCompany());
		        		
		        		for (Receipt receipt : receiptArray) {
							try {
								receipt.setAccountId(purchaseordedetail.get(0).getAccountId()+"|"+purchaseordedetail.get(0).getGlOffSet());
								receipt.setLineType(purchaseordedetail.get(0).getLineType());
								receipt.setLineNumber((int)Math.floor(purchaseordedetail.get(0).getLineNumber()));
							} catch (Exception e) {
		        		...
		        		po.setOrderType("OP");
		        		po.setExchangeRate(poOriginal.getExchangeRate());
		        		...
		        		@SuppressWarnings("unused")
						VoucherHeaderDTO voucher = createNewVoucher(po,inv,0,s,receiptArrayNC.isEmpty()?receiptArray:receiptArrayNC,AppConstants.NN_MODULE_VOUCHER,fisDoc);
		            ...
		            for (int j = 0; j < objList.size(); j++) {
		            	// System.out.println("Guardado:" + objList.get(j).getSYEDOC());
							...
							List<FiscalDocuments> fisList = fiscalDocumentService.getReplicationInvoiceForUpdate(String.valueOf(objList.get(j).getSYAN8()), 
							 ...
							 fisList.addAll( fiscalDocumentService.getReplicationInvoiceForUpdate(String.valueOf(objList.get(j).getSYAN8()), 
									AppConstants.STATUS_PENDING_REPLICATION, objList.get(j).getSYDOCO(),
									AppConstants.NC_FIELD_UDC,AppConstants.STATUS_ACCEPT,/*objList.get(j).getSYVINV(),*/ 0,10));
	...
	public static String takeOffBOM(InputStream inputStream) throws IOException {
	  ...
	  public VoucherHeaderDTO createNewVoucher(PurchaseOrder o, InvoiceDTO inv, int nextNbr, Supplier s, List<Receipt> receipts, String nextNumberType , FiscalDocuments fisDoc){		
			...
			if(nextNbr == 0) {
				...
				linenumber=usuarioaprob.getLinenumber();
				nextNbrBatch=usuarioaprob.getBatchnumber();
...
//			}
			...
			recHdr.setSYEDBT((nextNbrBatch==0?nextNbr:nextNbrBatch)+"");
			recHdr.setSYEKCO(o.getOrderCompany());
			recHdr.setSYEDCT("OP".equals(o.getOrderType()) ? "PD" : o.getOrderType());
			recHdr.setSYEDLN(linenumber);
			...
			/*System.out.println("****** Folio:" + inv.getFolio());
			if(inv.getFolio() != null && !"null".equals(inv.getFolio()) && !"NULL".equals(inv.getFolio()) ) {
				folio = inv.getFolio();
			}
			
			String serie = "";
			System.out.println("****** Serie:" + inv.getSerie());
			...
			recHdr.setSYVINV("OP".equals(o.getOrderType()) ? inv.getCfdiRelacionado().substring(0,15) : vinv);
			...
			for(Receipt d : receipts) {	
				if(d.getAmountReceived() != 0) {
					...
					dto.setSZEDBT((nextNbrBatch==0?nextNbr:nextNbrBatch)+"");
					dto.setSZEDCT("OP".equals(o.getOrderType()) ? "PD" : o.getOrderType());
					...
					dto.setSZVINV("OP".equals(o.getOrderType()) ? inv.getCfdiRelacionado().substring(0,15) : "");
					dto.setSZAID("OP".equals(o.getOrderType()) ? d.getAccountId().split("\\|")[0]:d.getAccountId());//poner cuenta corta
					dto.setSZANI("OP".equals(o.getOrderType()) ? d.getAccountId().split("\\|")[1]:"");//poner la cuenta larga
					dto.setCRRM(o.getCurrencyMode());
					dto.setSZCRCD(o.getCurrecyCode());
					double unitcost=0;
					try {
						BigDecimal bdUte = new BigDecimal(inv.getConcepto().get(0).getValorUnitario());	
					 unitcost = bdUte.doubleValue();
					} catch (Exception e) {
						// TODO: handle exception
					}
					...
					dto.setSZUPRC(("OP".equals(o.getOrderType()) ? (unitcost*-1) : d.getUnitCost())   * 10000);
					dto.setSZAEXP(0);
					dto.setURAB(d.getReceiptLine());				
					dto.setSZUOPN(d.getQuantityReceived()*100);
					qtyOpen = qtyOpen + dto.getSZUOPN();
					dto.setSZVR01(inv.getUuid().substring(0, 25));
					dto.setSZVR02(usuarioaprob!=null?usuarioaprob.getUserName():"");
					dto.setSZURRF(inv.getUuid().substring(inv.getUuid().length() - 11));
					
					if(!AppConstants.DEFAULT_CURRENCY_JDE.equals(o.getCurrecyCode())) {
						BigDecimal bd = new BigDecimal(("OP".equals(o.getOrderType()) ? o.getOriginalOrderAmount() : d.getForeignAmountReceived())*100).setScale(2, RoundingMode.HALF_EVEN);
					...
					BigDecimal bd = new BigDecimal(("OP".equals(o.getOrderType()) ? o.getOriginalOrderAmount() : d.getAmountReceived())*100).setScale(2, RoundingMode.HALF_EVEN);
			...
			vs.setSWEDCT("OP".equals(o.getOrderType()) ? "PD" : o.getOrderType());
			vs.setSWDCTV("P4");
			if(!AppConstants.DEFAULT_CURRENCY_JDE.equals(o.getCurrecyCode())) {
	  ...
	  public Map<String, Object> getPostVoucherForeign(int start, int limit){
				...
				if (!fiscalDocList.isEmpty()) {
					...
					for (FiscalDocuments fisDoc : fiscalDocList) {
						if (fisDoc.getUuidFactura() != null && !"".equals(fisDoc.getUuidFactura())) {
			        		...
			        		@SuppressWarnings("unused")
			        		VoucherHeaderDTO voucher =  EDIService.createNewForeignVoucherMdw(po, foreingInvoice, 0, s, receiptArray, AppConstants.NN_MODULE_VOUCHER,fisDoc);
		...
		public Map<String, Object> getListPdfBatch(){
			try{
				List<String> userDocuments = userDocumentDao.getListPdfBatchNames();
				List<String> listFiles =userDocuments==null?new ArrayList<String>():userDocuments;
				String jsonInString = null;
//					for (UserDocument document : userDocuments) {
//						listFiles.add(document.getName());
//					}

				Gson gson = new Gson();  
				jsonInString = gson.toJson(listFiles); 
			    return mapOK(jsonInString,listFiles.size());
			} catch (Exception e) {
				log4j.error("Exception" , e);
				e.printStackTrace();
				return mapError(e.getMessage() + " - getListPdfBatch");
			}
		}
...
//				doc.setAddressBook(new PDFUtils().getPdfBatchNumber(input));
				...
				if(doc.getAddressBook() != null) {
					userDocumentDao.save(doc);
					log4j.info("Se guarda el archivo BATCH: " + name);
				} else {
//					log4j.error("No fue posible obtener el Batch Number del documento: " + name);
					return mapError("Sin batch");	
				}				
			} catch (Exception e) {
				log4j.error("Exception" , e);
				return mapError("Error: getListPdfBatch: "+e.getMessage());				
			}
		...
		public Map<String, Object> getVoucherDocs(){
			try{
				...
				int fileAttachedCount = 0;
				List<FileDTO> files = new ArrayList<FileDTO>();
				String[] fisTypes = new String[]{"Factura", "Evidencia", "Otros", "BATCHCOVER", "BATCH", "I","NotaCredito"};
				...
				if(userDocuments != null  && !userDocuments.isEmpty()) {
					...
					for (UserDocument document : userDocuments) {
						try {
							...
							if("NOTACREDITO".equals(fisType)) {
								//Para Notas de Crédito se envía el uuid de la factura relacionada
								vinv = document.getDescription() != null ? document.getDescription().replace("MainUUID_", "").substring(0,25):vinv;
							}	
							...
							if("".equals(currentAddressBook)) {
								document.setReplicationMessage("Documento Sin ADDRESSBOOK.");
								document.setReplicationDate(new Date());
								document.setReplicationStatus(AppConstants.STATUS_NOTSENT_REPLICATION);
								userDocumentDao.update(document);
						...
						if(portalPO != null) {							
							...
							try {
								...
								if(s!=null) {
									...
									//Multiusuarios
									String emailUsers = userService.getMailFromUsersByAddressNumber(portalPO.getAddressNumber());
							...
							if(r.getAddressNumber() != null && !r.getAddressNumber().trim().isEmpty()) {
								...
								if(!receiptSentList.contains(receiptString)) {
									...
									if(s!=null) {
										...
										//Multiusuarios
										String emailUsers = userService.getMailFromUsersByAddressNumber(r.getAddressNumber());
		...
		public Map<String, Object> savePaymentsSupplierDetail(List<HashMap<String, Object>> dataList) {
		 ...
		 public Map<String, Object> sendNotificacionPendienteContabilizar(String json) {
		        try {
		            JSONArray jsonArray = new JSONArray(json); // Convertir el string JSON a un array
		            
		            List<UDC> udcsMails = this.udcService.searchListBySystemAndKey("ORDER","MAILPENDEX");
		            ...
		            for (int i = 0; i < jsonArray.length(); i++) {
		                ...
		                String userName = obj.getString("usuario"); // Obtener usuario
		                JSONArray pendientes = obj.getJSONArray("pendientes"); // Obtener pendientes

		                // Buscar usuario en BD
		                Users usuario = userService.getByUserName(userName);
		                if (usuario == null || usuario.getEmail() == null) {
		                    log4j.error("Usuario no encontrado o sin email: " + userName);
		                    continue; // Saltar a la siguiente iteración si no se encuentra
		                }

		                StringBuilder emailBody = new StringBuilder();
		                emailBody.append("<h3>Estimado ").append(usuario.getName()).append(",</h3>");
		                emailBody.append("<p>Se le informa que tiene pendiente de Contabilizar los siguientes Batch:</p>");
		                emailBody.append("<table border='1' style='border-collapse: collapse; width: 100%;'>");
		                emailBody.append("<tr><th>Batch Number</th><th>Documentos</th><th>Fecha</th></tr>");

		                for (int j = 0; j < pendientes.length(); j++) {
		                    String[] datos = pendientes.getString(j).split("\\|"); // Separar por "|"
		                    
		                    if (datos.length == 3) { // Validar que existan las 3 columnas
		                        emailBody.append("<tr>");
		                        emailBody.append("<td>").append(datos[1]).append("</td>"); // Batch Number
		                        emailBody.append("<td>").append(datos[0]).append("</td>"); // cantidad
		                        emailBody.append("<td>").append(datos[2]).append("</td>"); // Fecha
		                        emailBody.append("</tr>");
		                    } else {
		                        log4j.error("Formato incorrecto en pendiente: " + pendientes.getString(j));
		                    }
		                }

		                emailBody.append("</table>");
		                emailBody.append("<p>Por favor, revisa estos pendientes.</p>");
		                // Enviar correo
		                EmailServiceAsync emailAsyncSup = new EmailServiceAsync();
		                emailAsyncSup.setProperties("Batchs Pendientes de Contabilizar - Documentos Fiscales", 
		                                            stringUtils.prepareEmailContent(emailBody.toString()), 
		                                            usuario.getEmail(),mails);
		                emailAsyncSup.setMailSender(mailSenderObj);
		                Thread emailThreadSup = new Thread(emailAsyncSup);
		                emailThreadSup.start();
		            }

		            return mapOK("Correos enviados correctamente", 0);

		        } catch (Exception e) {
		            log4j.error("Exception", e);
		            return mapError("sendNotificacionPendienteContabilizar: " + e.getMessage());
		        }
		    }
